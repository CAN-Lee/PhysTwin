...
class WarpMPMSimulator(nn.Module):
    ...
    def step(self, moe_params_wp, controller_pos=None, controller_vel=None, residual_v=None, damping_override=None, stiffness_override=None):
        """
        Execute one MPM step.
        """
        # Execute logic once, using tape if available
        def _logic():
            ...
            if self.controller_indices_wp is not None and controller_pos is not None:
                ...
                kp = float(stiffness_override) if stiffness_override is not None else float(getattr(self.cfg, 'controller_stiffness', 1000.0))
                kd = float(getattr(self.cfg, 'controller_damping', 0.0))
                ...
                wp.launch(accumulate_pd_forces_kernel, 
                          ...
                          inputs=[self.particle_dv_wp, self.solver.mpm_state.particle_x, self.solver.mpm_state.particle_v,
                                  ...
                                  self.initial_offsets_wp, self.count_per_particle_wp, kp, kd, self.dt],
                          device=self.warp_device)
                wp.launch(apply_dv_with_clamping_kernel, self.n_particles, 
                          [self.solver.mpm_state.particle_v, self.particle_dv_wp, clamp_val],
                          device=self.warp_device)

            # B. Inject Neural Damping/Residual
            if damping_override is not None:
                damping_wp = torch2warp_float(1.0 - damping_override)
                wp.launch(mul_vec3_vec3_elementwise, self.n_particles, [self.solver.mpm_state.particle_v, damping_wp], device=self.warp_device)
        ...
        # 4. Export results
        # [FIX] Ensure these are clean leaf tensors for PyTorch to avoid "non-leaf grad" warnings
        # and ensure gradients are correctly captured for tape.backward()
