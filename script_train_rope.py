...
def worker(gpu_id, task_queue, config_path, iters):
    ...
    while True:
        ...
        selected_config = config_path if config_path else "configs/rope.yaml"
        print(f"\n[GPU {gpu_id}] >>> [ROPE MODE] Training Scene: {scene_id} using {selected_config}", flush=True)
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/rope.yaml", help="Path to config file")
    args = parser.parse_args()
    
    from omegaconf import OmegaConf

    scenes_paths = sorted(glob.glob(os.path.join(args.base_path, "*")))
    # Filter only rope scenes
    valid_scenes = [os.path.basename(p) for p in scenes_paths if os.path.isdir(p) and "rope" in os.path.basename(p).lower()]
    ...
    print(f"Found {len(valid_scenes)} ROPE scenes. Using GPU(s): {args.gpus}. Config: {args.config}", flush=True)
