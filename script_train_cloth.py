...
def worker(gpu_id, task_queue, config_path, iters):
    ...
    while True:
        ...
        selected_config = config_path if config_path else "configs/mpm_cloth.yaml"
        print(f"\n[GPU {gpu_id}] >>> [CLOTH MODE] Training Scene: {scene_id} using {selected_config}", flush=True)
...
def main():
    ...
    parser.add_argument("--config", type=str, default="configs/mpm_cloth.yaml", help="Path to config file")
    args = parser.parse_args()

    from omegaconf import OmegaConf

    scenes_paths = sorted(glob.glob(os.path.join(args.base_path, "*")))
    # Filter cloth/package scenes, excluding rope
    valid_scenes = [os.path.basename(p) for p in scenes_paths if os.path.isdir(p) and 
                    any(kw in os.path.basename(p).lower() for kw in ['cloth', 'package']) and 
                    "rope" not in os.path.basename(p).lower()]
    ...
    if os.path.exists(args.config):
        try:
            ...
            if 'target_scenes' in cfg and cfg.target_scenes:
                ...
                # Intersect found scenes with targets to ensure they exist
                valid_scenes = [s for s in valid_scenes if s in targets]
                if len(valid_scenes) != len(targets):
                    print(f"WARNING: Some target scenes were not found in {args.base_path}")
        except Exception as e:
            print(f"WARNING: Failed to parse target_scenes from {args.config}: {e}")

    print(f"Found {len(valid_scenes)} CLOTH scenes. Using GPU(s): {args.gpus}. Config: {args.config}", flush=True)
    ...
    try:
        ...
        os._exit(1)
    print("\n[ALL DONE] All cloth scenes processed.", flush=True)
